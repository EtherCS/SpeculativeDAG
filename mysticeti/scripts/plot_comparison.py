...
for i, csv_path in enumerate(csv_files):
    ...
    ax.plot(x, df['p50_ms'].to_numpy() / 1000.0, linewidth=1.5, color=color, linestyle='-',  marker=marker, markersize=4, label=f'{label} (p50)')
    ax.plot(x, df['p90_ms'].to_numpy() / 1000.0, linewidth=1.5, color=color, linestyle='--', marker=marker, markersize=4, label=f'{label} (p90)')

ax.set_xlabel('Request start time (s)')
ax.set_ylabel('Transaction Committed Latency (s)')
