...
impl PevmAPI {
    pub fn new(
        ...
        let addresses = load_account_addresses(&workload_type, account_addresses_path);
        let in_memory_storage = load_in_memory_storage(&workload_type, account_storage_path);
        Self {
            pevm: Pevm::default(),
...
impl PevmExecutor {
    pub fn new(
        ...
        Self {
            execution_mode,
            storage: load_in_memory_storage(&workload_type, account_storage_path),
...
impl PevmTransactionGenerator {
    pub fn new(
        ...
        replica_id: u64,
        replica_num: u64,
        pevm_txn_sender: mpsc::Sender<Vec<(String, Address)>>,
        insufficient_txn_signal_receiver: mpsc::Receiver<usize>,
        account_addresses_path: String,
    ) -> Self {
        let clusters = load_account_addresses(&workload_type, account_addresses_path);
        tracing::debug!("Loaded {:?} clusters", clusters);
    ...
    pub fn generate_transactions(&mut self) -> Vec<(String, Address)> {
        match self.workload_type {
            WorkloadType::ERC20(_, _, _) => match self.high_contention_interval {
    ...
    fn generate_contended_erc20_transactions(&mut self) -> Vec<(String, Address)> {
        ...
        let three_random_families: Vec<usize> = three_random_clusters
            .iter()
            .map(|&cluster_idx| {
    ...
    fn generate_parallelizable_weth_transactions(&mut self) -> Vec<(String, Address)> {
        ...
        for (cluster_id, (weth_address, families)) in self.clusters.iter().enumerate() {
            for (family_id, family) in families.iter().enumerate() {
                ...
                // transferFrom transactions in a family share one operator account.
                // Keep that nonce owner and all of its users on one replica.
...
impl PevmScheduler {
    pub fn new(pevm_txn_receiver: mpsc::Receiver<Vec<(String, Address)>>) -> Self {
        Self {
    ...
    pub fn schedule_a_parallelizable_batch(
        txns: &mut Vec<TransactionWithHint>,
    ) -> Vec<Option<TransactionWithHint>> {
    ...
    pub fn schedule_a_parallelizable_batch_fair_sequential(
        txns: &mut Vec<TransactionWithHint>,
    ) -> Vec<Option<TransactionWithHint>> {
        let concurrency_level: usize = thread::available_parallelism()
            .unwrap_or(NonZeroUsize::MIN)
        ...
        let caller_queues = Arc::new(DashMap::<
            Address,
            Arc<StdMutex<VecDeque<(usize, TransactionWithHint)>>>,
        >::new());
                    ...
                    loop {
                        ...
                        match caller {
                                    ...
                                    if let Some(queue_ref) = caller_queues.get(&caller) {
                                        ...
                                        if let Some((index, txn)) = queue.pop_front() {
                                            ...
                                            if available_slots.load(Ordering::Acquire) > 0 {
                                                position_found =
                                                    Self::process_transaction_sequential(
                                                        thread_id,
                                                        index,
                                                        &txn,
                                                        concurrency_level,
                                                ...
                                                if !position_found {
                                                    ...
                                                    queue.push_front((index, txn));
                                                    // Re-queue if not scheduled
                                                }
                                            }
                            ...
                            None => {
                                ...
                                let has_remaining_work = caller_queues
                                    .iter()
                                ...
                                if backoff.is_completed() {
                                    ...
                                    for caller_entry in caller_queues.iter() {
                                        ...
                                        if !queue.is_empty()
                                            && !caller_processing.contains_key(caller_entry.key())
                                        {
                                            injector.push(*caller_entry.key());
                                        }
                                    }
...
pub fn test_load_both() {
    ...
    let mut generator =
        PevmTransactionGenerator::new(workload_type, 1u64, 4u64, tx1, rx2, address_bin);
...
pub fn test_load_in_memory_storage() {
    let workload_type = WorkloadType::ERC20(1, 1, 4);
    let account_storage_path = "storage_1_1_4".to_string();
    let account_addresses_path = "account_addresses_1_1_4.bin".to_string();
    let storage = load_in_memory_storage(&workload_type, account_storage_path);
    let addresses = load_account_addresses(&workload_type, account_addresses_path);
    println!("{:?}", addresses);
}
...
pub fn test_max_throughput_sequential() {
    ...
    let mut generator =
        PevmTransactionGenerator::new(workload_type, 1u64, 4u64, tx1, rx2, address_bin.clone());
...
pub fn test_scheduling() {
    let workload_type = WorkloadType::ERC20(8, 1, 8);
    let account_storage_path = "storage_8_1_8.json".to_string();
    let account_addresses_path = "account_addresses_8_1_8.bin".to_string();
    let mut restored_storage = load_in_memory_storage(&workload_type, account_storage_path);
    let chain = PevmEthereum::mainnet();
...
pub fn test_parallel_scheduling() {
    ...
    while !transactions_with_hint.is_empty() {
        ...
        let batch = PevmScheduler::schedule_a_parallelizable_batch_fair_sequential(
            &mut transactions_with_hint,
        );
...
pub fn test_no_scheduling() {
    ...
    while !transactions_with_hint.is_empty() {
        ...
        let tx_envs: Vec<TxEnv> = batch
            .iter()
            .map(|txn| deserializer::decode_one_hex(txn.raw_hex.clone(), txn.caller).unwrap())
            .collect();
...
pub fn test_contended_workload() {
    ...
    let account_storage_path = "storage_8_1_8.json".to_string();
    let account_addresses_path = "account_addresses_8_1_8.bin".to_string();
    let restored_storage = load_in_memory_storage(&workload_type, account_storage_path);
    let chain = PevmEthereum::mainnet();
    let mut generator = PevmTransactionGenerator::new(
        workload_type,
        0u64,
    ...
    println!(
        "Executed trasactions in parallel with {} threads",
        concurrency_level
    );
